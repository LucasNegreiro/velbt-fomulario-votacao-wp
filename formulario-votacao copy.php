...
function meu_formulario_votacao_shortcode() {
    ...
    <script>
        jQuery(document).ready(function($) {
            console.log('Documento pronto');  // Verifique se o documento está pronto

           
            var $empresaNome = $('#empresa_nome');

            $empresaNome = $('#empresa_nome').select2({
                language: {
                ...
                placeholder: 'Selecione uma empresa', // Adicionado um placeholder
                ajax: {
                    url: '<?php echo admin_url('admin-ajax.php'); ?>',
                    dataType: 'json',
                    delay: 250,
                    data: function (params) {
                        console.log('Função de dados chamada');  // Verifique se esta função está sendo chamada
                        console.log('Params antes da query:', params);  // Log do objeto params completo

                        var query = {
                            action: 'buscar_empresas',
                            term: params.term || '',  // Use um valor padrão se o termo estiver indefinido
                            page: params.page || 1
                        };

                        console.log('Query sendo enviada:', query);  // Log da query que será enviada

                        return query;
                    },
                    processResults: function (data) {
                        console.log('Data recebida:', data);  // Log da data recebida
            ...
            if ($empresaNome.data('select2')) {
                console.log('Select2 foi inicializado');  // Verifique se o Select2 foi inicializado
            } else {
            ...
            $empresaNome.on('select2:select', function (e) {
                console.log('Evento select2:select disparado');  // Verifique se este log aparece quando você seleciona um item
                console.log('Objeto do evento:', e);  // Log do objeto do evento completo para verificar se contém os dados esperados
